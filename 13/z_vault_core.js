...
console.log("🛡️ Z-Vault Core v2.5.0 Active for Z-TORUS RIFE PEMF");
...
function continueToApp() {
    const pending = localStorage.getItem('z_vault_pending_url_rife');
    if (pending && pending !== 'index.html') {
        localStorage.removeItem('z_vault_pending_url_rife');
...
function showDisclaimerUI() {
    ...
    btn.onclick = () => {
        localStorage.setItem('z_vault_disclaimer_accepted_rife', 'true');
        overlay.remove();
        continueToApp();
    };
}
...
function getDeviceID() {
    let id = localStorage.getItem('z_vault_device_id_rife');
    if (!id || !id.startsWith('RIFE-')) {
        ...
        id = "RIFE-" + code;
        localStorage.setItem('z_vault_device_id_rife', id);
        let oldKey = localStorage.getItem('z_vault_active_key');
        if (oldKey) localStorage.setItem('z_vault_active_key_rife', oldKey);
        let oldDisclaimer = localStorage.getItem('z_vault_disclaimer_accepted');
        if (oldDisclaimer) localStorage.setItem('z_vault_disclaimer_accepted_rife', oldDisclaimer);
    }
    return id;
}
...
window.ZVault = {
    isActivated: function() {
        const key = localStorage.getItem('z_vault_active_key_rife');
        if (!key) return false;
        return verifyKey(key);
    },
    isDisclaimerAccepted: function() {
        return localStorage.getItem('z_vault_disclaimer_accepted_rife') === 'true';
    },
    init: function() {
        if (this.isActivated()) {
    ...
    injectUI: function() {
        ...
        const html = `
            <div id="zVaultOverlay" style="position:fixed;inset:0;background:#0D0F21;z-index:10000;display:flex;align-items:center;justify-content:center;color:white;font-family:sans-serif;">
                <div style="background:rgba(255,255,255,0.05);padding:40px;border-radius:30px;border:1px solid rgba(255,255,255,0.1);max-width:350px;width:90%;text-align:center;">
                    <h2 style="color:#FFD700;margin-bottom:10px;">Z-TORUS RIFE PEMF</h2>
        ...
        document.getElementById('zBtn').onclick = async function() {
            ...
            if (verifyKey(key)) {
                ...
                localStorage.setItem('z_vault_active_key_rife', key);
...
window.handleAppClick = function(e) {
    if (!window.ZVault.isActivated() || !window.ZVault.isDisclaimerAccepted()) {
        ...
        if (url) localStorage.setItem('z_vault_pending_url_rife', url);
        window.ZVault.init();
        return false;
    }
    return true;
