...
const apps = [
    { folder: 'standart', suffix: 'std', title: 'Z-TORUS STANDART' },
    { folder: 'rife', suffix: 'rife', title: 'Z-TORUS RIFE PEMF' },
    { folder: 'rodin', suffix: 'rodin', title: 'Z-TORUS RODIN PEMF' },
    { folder: 'wav-tool', suffix: 'wav', title: 'Z-TORUS WAV EXPORT' }
];

const template = (suffix, title) => `/**
 * Z-Vault Core v2.5.0 - DISCLAIMER ENFORCED EDITION
 */

// 1. GLOBAL CONSTANTS
const MASTER_SALT = "Z-TORUS-VULT-SAFE-@2024!";
console.log("🛡️ Z-Vault Core v2.5.0 Active for ${title}");
...
const i18nVault = {
    tr: {
        ...
        disclaimer_html: \`<h3 style="color:#FFD700;text-align:center;">⚠️ GÜVENLİK UYARISI</h3><p>Z-Torus Sistemi güvenli bir teknolojidir. Ancak aşağıdaki durumlarda <strong>cihazı kullanmamalısınız:</strong></p><ul><li>Kalp pili veya elektronik tıbbi implantı olanlar.</li><li>Hamileler veya gebelik şüphesi olanlar.</li><li>Epilepsi (sara) geçmişi olanlar.</li><li>Yeni organ nakli yaptıranlar.</li></ul><p><b>Yasal Uyarı:</b> Bu uygulama tıbbi bir cihaz değildir. Teşhis veya tedavi amacı gütmez. Tüm sorumluluk kullanıcıya aittir.</p>\`
    },
    en: {
...
function continueToApp() {
    const pending = localStorage.getItem('z_vault_pending_url_${suffix}');
    if (pending && pending !== 'index.html') {
        localStorage.removeItem('z_vault_pending_url_${suffix}');
...
function showDisclaimerUI() {
    ...
    btn.onclick = () => {
        localStorage.setItem('z_vault_disclaimer_accepted_${suffix}', 'true');
        overlay.remove();
        continueToApp();
    };
}
...
function getDeviceID() {
    let id = localStorage.getItem('z_vault_device_id_${suffix}');
    if (!id) {
        let oldId = localStorage.getItem('z_vault_device_id');
        if (oldId && oldId.startsWith("ZT-")) {
            id = oldId;
            localStorage.setItem('z_vault_device_id_${suffix}', id);
            let oldKey = localStorage.getItem('z_vault_active_key');
            if (oldKey) localStorage.setItem('z_vault_active_key_${suffix}', oldKey);
            let oldDisclaimer = localStorage.getItem('z_vault_disclaimer_accepted');
            if (oldDisclaimer) localStorage.setItem('z_vault_disclaimer_accepted_${suffix}', oldDisclaimer);
        } else {
            let prefix = "${suffix.toUpperCase()}-";
            id = prefix + Math.random().toString(36).substring(2, 10).toUpperCase();
            localStorage.setItem('z_vault_device_id_${suffix}', id);
        }
    }
    return id;
...
function verifyKey(inputKey) {
    try {
        ...
        const decrypted = xorCipher(decoded, MASTER_SALT);
        const parts = decrypted.split('|');
        if (parts.length < 2) return false;
        
        const keyID = parts[0].trim().toUpperCase();
        const currentID = getDeviceID().trim().toUpperCase();
...
window.ZVault = {
    isActivated: function() {
        const key = localStorage.getItem('z_vault_active_key_${suffix}');
        if (!key) return false;
        return verifyKey(key);
    },
    isDisclaimerAccepted: function() {
        return localStorage.getItem('z_vault_disclaimer_accepted_${suffix}') === 'true';
    },
    init: function() {
        // If activated but NO disclaimer, show disclaimer directly
        if (this.isActivated()) {
    ...
    injectUI: function() {
        ...
        const html = \`
            <div id="zVaultOverlay" style="position:fixed;inset:0;background:#0D0F21;z-index:10000;display:flex;align-items:center;justify-content:center;color:white;font-family:sans-serif;">
                <div style="background:rgba(255,255,255,0.05);padding:40px;border-radius:30px;border:1px solid rgba(255,255,255,0.1);max-width:350px;width:90%;text-align:center;">
                    <h2 style="color:#FFD700;margin-bottom:10px;">${title}</h2>
        ...
        document.getElementById('zBtn').onclick = async function() {
            ...
            if (verifyKey(key)) {
                ...
                localStorage.setItem('z_vault_active_key_${suffix}', key);
...
window.handleAppClick = function(e) {
    if (!window.ZVault.isActivated() || !window.ZVault.isDisclaimerAccepted()) {
        ...
        if (url) localStorage.setItem('z_vault_pending_url_${suffix}', url);
        window.ZVault.init();
        return false;
    }
    return true;
...
for (const app of apps) {
    const filePath = path.join(__dirname, app.folder, 'z_vault_core.js');
    fs.writeFileSync(filePath, template(app.suffix, app.title));
